...
assert_contains "$SERVER_DIR/src/index.ts" "'en time før' betyder 60 minutter" "one-hour reminder instruction"
...
assert_contains "$SERVER_DIR/src/index.ts" "formiddag uden klokkeslæt betyder start kl. 09:00" "approximate time period defaults"
assert_contains "$SERVER_DIR/src/index.ts" "Brug ikke 'i morgen' alene som tidsperiode" "tomorrow date still requires start time"
assert_contains "$SERVER_DIR/src/index.ts" "lavere confidence end 0.90" "approximate time periods require review"
assert_contains "$SERVER_DIR/src/index.ts" "'halv otte' = 07:30 eller 19:30" "Danish half-hour clock instruction"
assert_regex "$SERVER_DIR/src/index.ts" "hver anden tirsdag|hver uge|hver måned" "Danish recurrence examples"
assert_contains "$SERVER_DIR/src/index.ts" "hver tredje uge" "Danish recurrence interval instruction"
...
assert_contains "$ROOT_DIR/ios/VoiceCalendarAssistant/VoiceCalendarAssistant/VoiceAssistantViewModel.swift" "approximatePeriodWithoutExplicitTime" "auto-save reviews approximate period times"
